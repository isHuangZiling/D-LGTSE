...
class ERB(nn.Module):
    def __init__(self, erb_subband_1, erb_subband_2, nfft=256, high_lim=4000, fs=8000):
        super().__init__()
        erb_filters = self.erb_filter_banks(erb_subband_1, erb_subband_2, nfft, high_lim, fs) # (64,192)
        nfreqs = nfft//2 + 1 #257
    ...
    def erb_filter_banks(self, erb_subband_1, erb_subband_2, nfft=256, high_lim=4000, fs=8000):
        low_lim = erb_subband_1/nfft * fs #erb_subband_1=65,erb_subband_1=64,low_dim=2031.25
        erb_low = self.hz2erb(low_lim) #21.28
        erb_high = self.hz2erb(high_lim) #33.29
        erb_points = np.linspace(erb_low, erb_high, erb_subband_2) # 21.28 33.29  64,shape(64,) array
...
class SFE(nn.Module):
    ...
    def __init__(self, kernel_size=3, stride=1):
        ...
        self.unfold = nn.Unfold(kernel_size=(1,kernel_size), stride=(1, stride), padding=(0, (kernel_size-1)//2)) #展开
...
if __name__ == "__main__":
    ...
    y1 = model(x1)[0]
    y2 = model(x2)[0]
    y1 = torch.complex(y1[..., 0], y1[..., 1])
    y2 = torch.complex(y2[..., 0], y2[..., 1])

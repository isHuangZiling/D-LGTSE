...
def write_wav(fname, samps, fs=8000, normalize=True):
    """
    ...
    if fdir and not os.path.exists(fdir):
        os.makedirs(fdir)
    sf.write(fname, samps, fs, subtype='FLOAT',format='WAV')
...
class wespeakerenc(Reader):
    
    def __init__(self, scp_path): 
        super(wespeakerenc, self).__init__(scp_path)
        pass
        
        
    def _load(self, key):
    
        path = self.index_dict[key]
        
        # x = torch.load('{path}'.format(path=path)) #.pt
        
        x = kaldiio.load_mat('{path}'.format(path=path)) # kaldiio ark
        x = torch.tensor(x)
        x = torch.unsqueeze(x, 0)
        
        # x = np.load('{path}'.format(path=path)) #.npy   
        # assert x.dim() == 2  
        # x = torch.tensor(x)

...
class DenseUNet(nn.Module):
    ...
    def _build_encoder(self, **enc_kargs):
        ...
        encoder.append(nn.Sequential(DenseBlock(16, 16, "enc"),LCA(16)))
        for i in range(3):
            encoder.append(
                    nn.Sequential(
    ...
    def forward(self, 
        ...
        if mix.dim() == 1:
            ...
            se_out = th.unsqueeze(se_out, 0)
            enrollment = th.unsqueeze(enrollment, 0)   
        mix_spec = self.wav2spec(mix, False)
        se_out_spec = self.wav2spec(se_out, False)
        mix_spec_change = self.FeaCompression(mix_spec)
        se_out_spec_change = self.FeaCompression(se_out_spec)
        # similarity = []
        similarityseout = []
        aux_drc = []
        for i in range(batch_size):
            ...
            similarityseout.append(th.cat([self.ComputeSimilarity(se_out_spec_change[i, 0, ...], aux_spec_change[0, 0, ...]), self.ComputeSimilarity(se_out_spec_change[i, 1, ...], aux_spec_change[0, 1, ...])], dim=1))
        similarityseout = th.cat(similarityseout, dim=0)
        fus = th.cat((mix_spec_change, similarityseout), dim=1)
...
if __name__ == "__main__":
    ...
    mix = th.randn(2, 8000)
    aux = th.randn(2, 8000)
    se_out = th.randn(2, 8000)
    est = nnet(mix, se_out, aux)
    macs, params = profile(nnet, inputs=(mix,se_out, aux))

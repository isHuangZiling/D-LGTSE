...
class FreqNet(nn.Module):
    ...
    def forward(self, input, enrollment):
        if enrollment.dim() == 2:
        ...
        for i in range(batch_size):
            ...
            similarity.append(torch.cat([self.ComputeSimilarity(output[i, 0, ...], temp[0, 0, ...]), self.ComputeSimilarity(output[i, 1, ...], temp[0, 1, ...])], dim=1))
